...
export const Dashboard: React.FC = () => {
  ...
  const characterCount = useLiveQuery(() => db.characters.count()) ?? 0;
  const backgroundCount = useLiveQuery(() => db.backgrounds.count()) ?? 0;
  const storyCount = useLiveQuery(() => db.stories.count()) ?? 0;
  const videoTasks = useLiveQuery(() => db.videoTasks.toArray());

  const taskStats = React.useMemo(() => {
    if (!videoTasks) return { success: 0, failed: 0, processing: 0, total: 0 };
    return {
      success: videoTasks.filter(t => t.status === 'SUCCESS').length,
      failed: videoTasks.filter(t => t.status === 'FAILED').length,
      processing: videoTasks.filter(t => t.status === 'PROCESSING' || t.status === 'PENDING').length,
      total: videoTasks.length
    };
  }, [videoTasks]);

  const recentStories = useLiveQuery(
    () => db.stories.orderBy('createdAt').reverse().limit(3).toArray()
  );
